...
# create / start image processing for plot4
png(filename='Course Project 1/plots/plot4.png',width=480,height=480,units='px')
# create 4 different plots (2x2)
par(mfrow=c(2,2))
# plot top left (1,1)
plot(mydt[, DateTime], mydt[, Global_active_power], ylab='Global Active Power',xlab='',type='l')
# plot top right (1,2)
plot(mydt[, DateTime], mydt[, Voltage], xlab='datetime', ylab='Voltage',type='l')
...
legend('topright',legend=labels,col=lineCol,lty='solid', bty = "n")
# plot bottom right (2,2)
plot(mydt[, DateTime], mydt[, Global_reactive_power],xlab='datetime',ylab='Global_reactive_power',type='l')
